...
public:
    ...
    virtual void behavior(player *p, monster** monster_list, int num_of_monsters){};
};
...
public:
        ...
        //this->hp -= std::max(0, abs(this->armor - hp));
            std::cout << "player is damaged, hp left: " << this->hp <<" " << this->armor <<" " << damag<< std::endl;
            if (this->armor < damag) {
                std::cout << "damag - armor " << (damag - this->armor) << std::endl;
...
protected:
    ...
    int attack_radius;
public:
    bomb(int sx, int sy, int sp, int l) {
        std::cout << "Bomb created!" << std::endl;
        this->type_monster = 3;
        this->x = sx; //sx - start x
        this->y = sy;
        this->speed = sp;
        this->distance = 0; // distance - between start point and current point
        this->limit = l; // distance where bomb explodes
        this->hp = 1;
        this->attack_radius = 50;
    }
    ~bomb() {}
    void behavior(player *p, monster** monsters_list, int num_of_monsters) {
        std::cout << "BOMB behavior" << std::endl;
        this->x += speed;
        this->distance += speed;
...
public:
    std::chrono::time_point<std::chrono::system_clock> last_hit;
    flight(int xp, int yp, int tx0, int ty0, monster** monsters_list, int num_of_monsters, bomb* b) {
        this->type_monster = 2;
        ...
        last_hit = std::chrono::system_clock::now();
        this->b_attack = b;
        std::cout << "flight created with period: " << this->period_bomb << std::endl;
    }
    void behavior(player *p, monster** monsters_list, int num_of_monsters) {
        std::cout << "flight behavior" << std::endl;
        ...
        if (counted >= this->period_bomb) {
            ...
            // creation of bombs
            //monsters_list[] = new bomb(this->x, this->y, 10, 10, 5);
            for (int i = num_of_monsters; i < num_of_monsters * 2; i++) {
                if (monsters_list[i] == nullptr) {
                    std::cout << "null pointer -> THE BOMB STARTS FALLING HERE!!!" << std::endl;
...
int main(){
	...
	f.close();
	std::ifstream fm("monsters_my.txt");
	int num_of_monsters;
	fm >> num_of_monsters;
	monster** monsters_list = new monster*[num_of_monsters * 2];
	...
	for(int i = 0; i < num_of_monsters; i++) {
        ...
        if (type == 1) {
            ...
            monsters_list[i] = new bomb(x0, y0, 5, 100);
            monsters_list[i + 1] = new flight(x0, y0, rad, dir, monsters_list, num_of_monsters, monsters_list[i]);
        }
		std::cout << "monsters created." << std::endl;
	}
	...
	int cx = 0, cy= 0;
    player* player_1 = new player(0,0,36,20, 30*cx, 30*cy, 7);
    ...
    while (window.isOpen())
    {
		...
		for(int i = 0; i < num_of_monsters * 2; i++){
            if(monsters_list[i]) {
                std::cout << monsters_list[i]->type_monster << std::endl;
                monsters_list[i]->behavior(player_1, monsters_list, num_of_monsters);
                if (monsters_list[i]->get_hp() == 0) {
                    monsters_list[i] = NULL;
                }
            }
        }
		...
		for(int i = 0; i < num_of_monsters; i++){
            if(monsters_list[i]){
                if(abs(player_1->get_x()-monsters_list[i]->get_x())+abs(player_1->get_y()-monsters_list[i]->get_y()) < 40*30){
                    ...
                    //std::cout << monsters_list[i]->get_type() << " type" << std::endl;
